...
describe("GET /api/cron/process-usage-events", () => {
  ...
  describe("usage event processing", () => {
    ...
    it("charges ceil(quantity × unit_price / unit_size)", async () => {
      // 3 tweet.reads @ $0.01 each → 30 credits
  ...
  describe("member cap evaluation", () => {
    it("disables member when usage exceeds cap", async () => {
      ...
      // `usage_event`. To simulate a member already over cap, seed a processed
      // credit_usage row above the cap; this new usage_event adds more spend
      // and triggers the re-evaluation which then disables the member.
      await insertTestUsagePricing({
        kind: "connector",
        provider: "x",
        category: "tweet.read",
      ...
      // Seed historical credit_usage at the cap so evaluateMemberCaps sees it
